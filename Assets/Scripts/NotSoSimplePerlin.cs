using UnityEngine;

public static class NotSoSimplePerlin
{
    public struct NoiseResult
    {
        public float value;    // Noise height
        public Vector2 gradient;  // Derivative: (dNoise/dx, dNoise/dy)
    }

    // Permutation table (repeat to avoid overflow)
    static readonly int[] permutation = {
        151,160,137,91,90,15,
        131,13,201,95,96,53,194,233,7,225,
        140,36,103,30,69,142,8,99,37,240,21,
        10,23,190, 6,148,247,120,234,75,0,26,
        197,62,94,252,219,203,117,35,11,32,57,
        177,33,88,237,149,56,87,174,20,125,136,
        171,168, 68,175,74,165,71,134,139,48,27,
        166,77,146,158,231,83,111,229,122,60,211,
        133,230,220,105,92,41,55,46,245,40,244,
        102,143,54, 65,25,63,161, 1,216,80,73,
        209,76,132,187,208,89,18,169,200,196,135,
        130,116,188,159,86,164,100,109,198,173,186,
        3,64,52,217,226,250,124,123,5,202,38,
        147,118,126,255,82,85,212,207,206,59,227,
        47,16,58,17,182,189,28,42,223,183,170,
        213,119,248,152, 2,44,154,163,70,221,153,
        101,155,167,43,172,9,129,22,39,253,19,
        98,108,110,79,113,224,232,178,185,112,104,
        218,246,97,228,251,34,242,193,238,210,144,
        12,191,179,162,241,81,51,145,235,249,14,
        239,107,49,192,214,31,181,199,106,157,184,
        84,204,176,115,121,50,45,127, 4,150,254,
        138,236,205,93,222,114,67,29,24,72,243,
        141,128,195,78,66,215,61,156,180,

        // Repeat the table again to avoid overflow
        151,160,137,91,90,15,
        131,13,201,95,96,53,194,233,7,225,
        140,36,103,30,69,142,8,99,37,240,21,
        10,23,190, 6,148,247,120,234,75,0,26,
        197,62,94,252,219,203,117,35,11,32,57,
        177,33,88,237,149,56,87,174,20,125,136,
        171,168, 68,175,74,165,71,134,139,48,27,
        166,77,146,158,231,83,111,229,122,60,211,
        133,230,220,105,92,41,55,46,245,40,244,
        102,143,54, 65,25,63,161, 1,216,80,73,
        209,76,132,187,208,89,18,169,200,196,135,
        130,116,188,159,86,164,100,109,198,173,186,
        3,64,52,217,226,250,124,123,5,202,38,
        147,118,126,255,82,85,212,207,206,59,227,
        47,16,58,17,182,189,28,42,223,183,170,
        213,119,248,152, 2,44,154,163,70,221,153,
        101,155,167,43,172,9,129,22,39,253,19,
        98,108,110,79,113,224,232,178,185,112,104,
        218,246,97,228,251,34,242,193,238,210,144,
        12,191,179,162,241,81,51,145,235,249,14,
        239,107,49,192,214,31,181,199,106,157,184,
        84,204,176,115,121,50,45,127, 4,150,254,
        138,236,205,93,222,114,67,29,24,72,243,
        141,128,195,78,66,215,61,156,180
    };

    public static NoiseResult NoiseWithDerivative(float x, float y)
    {
        // Find unit grid cell containing point
        int X = Mathf.FloorToInt(x) & 255;
        int Y = Mathf.FloorToInt(y) & 255;

        // Relative x, y within cell
        float xf = x - Mathf.Floor(x);
        float yf = y - Mathf.Floor(y);

        // Compute quintic fade curves and derivatives
        float u = Fade(xf);
        float v = Fade(yf);
        float du = FadeDerivative(xf);
        float dv = FadeDerivative(yf);



        // Hash coordinates of the corners
        // int aa = permutation[permutation[X] + Y];
        // int ab = permutation[permutation[X] + Y + 1];
        // int ba = permutation[permutation[X + 1] + Y];
        // int bb = permutation[permutation[X + 1] + Y + 1];
        int aa = permutation[permutation[X] + Y    ];
        int ab = permutation[permutation[X] + Y + 1];
        int ba = permutation[permutation[X+1] + Y    ];
        int bb = permutation[permutation[X+1] + Y + 1];

        // Gradients at corners
        float gradAA = Grad(aa, xf, yf);        
        float gradBA = Grad(ba, xf - 1, yf);
        float gradAB = Grad(ab, xf, yf - 1);
        float gradBB = Grad(bb, xf - 1, yf - 1);

        // // Blend results
        // float lerpX1 = Mathf.Lerp(gradAA, gradBA, u);
        // float lerpX2 = Mathf.Lerp(gradAB, gradBB, u);
        // float value = Mathf.Lerp(lerpX1, lerpX2, v);

        // // Derivative calculations
        // float dLerpX1 = gradBA - gradAA;
        // float dLerpX2 = gradBB - gradAB;

        // float dx = du * (dLerpX1 * (1f - v) + dLerpX2 * v);
        // float dy = dv * (lerpX2 - lerpX1);


        // Blend results
        float lerpX1 = Mathf.Lerp(gradAA, gradBA, u);
        float lerpX2 = Mathf.Lerp(gradAB, gradBB, u);
        float value = Mathf.Lerp(lerpX1, lerpX2, v);

        // Derivative calculations
        float dLerpX1 = gradBA - gradAA;
        float dLerpX2 = gradBB - gradAB;
        float dLerpY1 = gradAB - gradAA;
        float dLerpY2 = gradBB - gradBA;

        float dx = du * (dLerpX1 * (1f - v) + dLerpX2 * v) + (dLerpY1 * (1f - u) + dLerpY2 * u);
        float dy = dv * (dLerpY1 * (1f - u) + dLerpY2 * u) + (dLerpX1 * (1f - v) + dLerpX2 * v);





        NoiseResult result;
        result.value = (value + 1f) / 2f;  // Normalize to [0,1]
        result.gradient = new Vector2(dx, dy);
        return result;





        // // Add blended results from corners
        // float res = Mathf.Lerp(
        //                 Mathf.Lerp(Grad(aa, x, y), Grad(ba, x - 1, y), u),
        //                 Mathf.Lerp(Grad(ab, x, y - 1), Grad(bb, x - 1, y - 1), u),
        //             v);

        // return (res + 1f) / 2f; // Normalize to [0,1]
    }
    public static float Noise(float x, float y)
    {
        return NoiseWithDerivative(x, y).value;
    }

    private static float Fade(float t)
    {
        // Quintic smoothing
        return t * t * t * (t * (t * 6 - 15) + 10);
    }

    private static float FadeDerivative(float t)
    {
        return 30.0f * t * t * (t * (t - 2f) + 1f);
    }

    private static float Grad(int hash, float x, float y)
    {
        // Convert low 4 bits of hash code into 12 gradient directions
        int h = hash & 7; // only 8 directions
        float u = (h < 4) ? x : y;
        float v = (h < 4) ? y : x;
        return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
    }
}
